    ...
    def assignServers(self, data):
        with open(data[2].decode(), "w") as f :
            ...
            while i < len(data) :
                ...
                self.servers[j][1] -= self.cantLimit
                if self.cantRand <= 0:
                    # primero vamos a remover los que tiene poco espacio y luego vamos a reconstruir el SegmentTree
                    nAux = 0
                    cantRemove = 0
                    j = 0                    
                    while j < self.cant :
                        if self.servers[j][1] >= self.cantLimit :
                            self.servers[nAux] = self.servers[j]
                            nAux += 1
                        else:
                            cantRemove += 1
                        j += 1
                    self.cant -= cantRemove
